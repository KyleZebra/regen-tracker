...
// (Benötigt Variablen aus data.js und engine.js)
// ==========================================
...
function triggerBonusConfetti() {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        let fallDuration = Math.random() * 3000 + 2000;
        conf.animate([
...
function updateUI() {
    ...
    if (typeof renderSandboxManager === 'function') {
        try { renderSandboxManager(); } catch(e) { console.error("Render Sandbox Error", e); }
    }
}
...
function renderDashboard() {
    ...
    if (!isSandbox && !isLoggedToday && !res.isOpen && displayDebt > 0 && typeof simulateCycle === 'function') {
        ...
        cloneA.logs[todayStr] = { type: 'pause', s:0, a:0, m:0, mood:0, note:'Phantom' }; 
        let resA = simulateCycle(cloneA);

        let cloneB = JSON.parse(JSON.stringify(activeCycle)); 
        if(!cloneB.logs) cloneB.logs = {}; 
        cloneB.logs[todayStr] = { type: 'ausrutscher', t:1, isSmall: false, s:0, a:0, m:0, mood:0, note:'Phantom' }; 
